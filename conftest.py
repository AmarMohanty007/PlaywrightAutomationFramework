...
def pytest_addoption(parser):
    ...
    """
    def add_option_if_missing(*args, **kwargs):
        try:
    ...
    add_option_if_missing("--browser", default="chromium", help="Browser: chromium, firefox, webkit, msedge")
    add_option_if_missing("--headed", action="store_true", help="Run in headed (visible) mode")
    add_option_if_missing("--base-url", default="https://tutorialsninja.com/demo/", help="Base URL for tests")
    add_option_if_missing("--video", default="retain-on-failure", help="Record video: on, off, retain-on-failure")
    add_option_if_missing("--screenshot", default="only-on-failure", help="Take screenshot: on, off, only-on-failure")
    add_option_if_missing("--tracing", default="retain-on-failure", help="Tracing: on, off, retain-on-failure")
...
def browser_context(request):
    ...
    if browser_name.lower() == "chromium":
        ...
        browser = playwright.chromium.launch(channel="msedge", headless=not headed_flag)
